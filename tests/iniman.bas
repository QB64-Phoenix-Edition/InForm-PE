...
SELECT CASE LCASE$(COMMAND$(2))
    CASE "", "-read", "read", "-r", "r"
        ...
        IF _COMMANDCOUNT = 4 THEN
            PRINT Ini_ReadSetting(file$, COMMAND$(3), COMMAND$(4))
        ELSE
            DO
    ...
    CASE "-write", "write", "-w", "w"
        ...
        IF _COMMANDCOUNT >= 4 THEN
            Ini_WriteSetting file$, COMMAND$(3), COMMAND$(4), COMMAND$(5)
            PRINT Ini_ReadSetting(file$, COMMAND$(3), COMMAND$(4))
        ELSE
            Usage
        END IF
    CASE "-delete", "delete", "-d", "d"
        ...
        IF _COMMANDCOUNT = 3 THEN
            Ini_DeleteSection file$, COMMAND$(3)
        ELSEIF _COMMANDCOUNT = 4 THEN
            Ini_DeleteKey file$, COMMAND$(3), COMMAND$(4)
        ELSE
            Usage
        END IF
    CASE "-sort", "sort", "-s", "s"
        ...
        IF _COMMANDCOUNT = 3 THEN
            Ini_SortSection file$, COMMAND$(3)
        ELSE
            Usage
        END IF
    CASE "-move", "move", "-m", "m"
        ...
        IF _COMMANDCOUNT = 5 THEN
            Ini_MoveKey file$, COMMAND$(3), COMMAND$(4), COMMAND$(5)
        ELSE
            Usage
        END IF
